...
3. 关于隔离属性（com.apple.quarantine）：
   • 首选「星TAP ARLink」点对直传：文件是应用直接落盘，系统不打隔离标记，
     解压后 .app 也干净，双击走「右键打开」即可，不会报「已损坏」。
   • 浏览器 / 邮件下载会打隔离属性，解压可能提示「已损坏」。两种干净解法：
     (a) 解压别用系统「归档实用工具」：用 Keka（App Store 免费）或终端 unzip
         解压，命令行 unzip 不传播隔离属性，解出的 .app 直接干净；
     (b) 若已用系统解压报损坏，终端执行（app 名带空格加引号）：
         xattr -dr com.apple.quarantine "星TAP 极速搜索.app"
     再重试第 2 步。（-dr 递归删隔离属性；等价于更彻底的 xattr -cr）
   • 走 HTTP 直链分发时：让对方用 curl -LO <下载地址> 而非浏览器点击，
     curl 落盘不打隔离标记，更省事。
